...
async def predict_disease(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Prepare results
        is_unidentified = result.get('is_unidentified', False)
        
        # Save to Supabase (for all predictions)
        try:
            ...
            storage_path = f"{label}/{file_id}.{file_ext}"
            ...
            print(f"[SUCCESS] Saved prediction to Supabase: {file_id} - {label}")
        except Exception as db_error:
            print(f"[WARNING] Failed to save to Supabase: {db_error}")
            # Continue even if database save fails
        
        # Return response based on identification status
        if is_unidentified:
            return JSONResponse(
                status_code=200, # Still return 200 as the prediction process worked
                content={
                    "success": True,
