...
================================================================================
OPTIMIZED FOR: Intel Core i5-13420H (8 cores, 16GB RAM)
...
- Larger batch size for better parallelization
- Optimized image size (224x224)
- Higher learning rate for faster convergence
- Data prefetching to reduce I/O bottleneck
"""
...
# HYPERPARAMETERS - Optimized for Intel Core i5-13420H
# -------------------------------------------------------------------
...
print("=" * 80)
print("TOMATO LEAF DISEASE DETECTION - i5-13420H OPTIMIZED TRAINING")
print("=" * 80)
...
print(f"\n⏱️  Estimated training time: 20-40 minutes (faster than previous setup)")
...
print(f"   • Custom Layers: Dense(512) → Dense(256) → Dense({train_generator.num_classes})")
...
callbacks = [
    ...
    EarlyStopping(
        monitor='val_loss',
        patience=7,                # Stop if no improvement for 7 epochs (increased from 5)
...
# Each epoch: model processes all training images in batches of 32
...
print(f"\n📝 NEXT STEPS:")
print(f"   1. Backup your current model (if exists):")
print(f"      Copy: {MODEL_SAVE_PATH}")
print(f"      To: {BACKUP_MODEL_PATH}")
print(f"\n   2. Restart your backend API to load the new model")
print(f"\n   3. Test with tomato leaf images")
print(f"\n   4. Review training history at: {HISTORY_PATH}")

print(f"\n💡 Optimizations Applied:")
print(f"   ✓ Intel oneDNN optimizations enabled")
print(f"   ✓ 8-core CPU parallelization")
print(f"   ✓ Larger image size (224x224) for better accuracy")
print(f"   ✓ Optimized batch size (32) for 8-core CPU")
print(f"   ✓ Higher learning rate (0.001) for faster convergence")
print(f"   ✓ Enhanced data augmentation")
print(f"   ✓ Early stopping with patience=7")
print(f"   ✓ Learning rate reduction on plateau")

print(f"\n🎯 Expected Performance:")
print(f"   • Training time: ~20-40 minutes (60-80% faster than old config)")
print(f"   • Better accuracy due to larger image size")
print(f"   • More robust predictions on low-quality images")
print(f"   • Improved handling of poor lighting conditions")
